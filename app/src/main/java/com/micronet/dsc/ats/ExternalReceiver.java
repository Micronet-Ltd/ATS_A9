...
public class ExternalReceiver extends WakefulBroadcastReceiver {
    ...
    // PING from 3rd party applications
    ...
    // Replies from ResetRB
    public static final String EXTERNAL_BROADCAST_RESETRB_REPLY = "com.micronet.dsc.resetRB.replyReset";
    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        if (intent.getAction().equals(EXTERNAL_BROADCAST_RESETRB_REPLY)) {

            Log.d(TAG, "ACK received from ResetRB app");
            // Send intent to service (so service is started if needed)
            Intent iservice = new Intent(context, MainService.class);
            iservice.putExtra(EXTERNAL_BROADCAST_RESETRB_REPLY, 1);
            startWakefulService(context, iservice); // wakeful to make sure we don't power down before
