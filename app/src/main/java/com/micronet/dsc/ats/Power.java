...
public class Power {
    ...
    public static final int ALARM_ATS_RESTART_REQUEST_CODE = 123456787; // unique private ID for the restart-app alarm
    public static final int ALARM_IOS_RESTART_REQUEST_CODE = 123456786; // unique private ID for the restart-app alarm
    public static final int ALARM_VBS_RESTART_REQUEST_CODE = 123456785; // unique private ID for the restart-app alarm
...
//            }
    ...
    ///////////////////////////////////////////////////////////////////
    // hasRebooted()
    //  used to determine if we need to redetect what vehicle we are in
    //  returns true if there is a chance the system has rebooted since the last time we checked
    //  if there is a chance we rebooted then there is a chance we were unplugged and we'll need to check the vehicle
    ///////////////////////////////////////////////////////////////////
    public boolean hasRebooted() {


        long time_last_booted_ms = System.currentTimeMillis() - SystemClock.elapsedRealtime();
        long saved_time_last_booted_ms = service.state.readStateLong(State.LAST_BOOT_TIME);

        boolean has_rebooted = true;




        // if a current time is not valid, then the start time is going to be the initial RTC time,
        //  and we will have to assume we rebooted.

        if (!isTimeValid(time_last_booted_ms)) {
            Log.v(TAG, "Boot time (=" + time_last_booted_ms + ") not valid. Assuming we rebooted.");
            has_rebooted = true;
        } else {
            // if the currently rebooted time is the within a few seconds of the saved one, then we did not reboot
            if ((time_last_booted_ms > saved_time_last_booted_ms - 3000) &&
                    (time_last_booted_ms < saved_time_last_booted_ms + 3000)) {
                Log.v(TAG, "Boot time (=" + time_last_booted_ms  + ") has not changed. was " + saved_time_last_booted_ms + " , diff= " + (saved_time_last_booted_ms - time_last_booted_ms));
                has_rebooted = false;
            } else {
                Log.v(TAG, "Boot time (=" + time_last_booted_ms  + ") has changed. was " + saved_time_last_booted_ms + " , diff= " + (saved_time_last_booted_ms - time_last_booted_ms));
                has_rebooted = true;
            }
        }

        // remember the reboot time for the next time we check
        saveRebootTime();

        //Log.v(TAG, "conclusion: We've rebooted");
        return has_rebooted;
...
//                Log.v(TAG, "PowerDownTask()");
    ...
    //      also resets the time last boooted
    ...
    //      also used for deciding if we might have rebooted
    // Parameters
    ...
    public void restartAtsProcess(String restart_reason) {
        ...
        // Our alarm intent MUST be a broadcast, if we just start a service, then wake lock may not be
        //  acquired in time to actually start running code in the service.
        PendingIntent pi = PendingIntent.getBroadcast(service.context, ALARM_ATS_RESTART_REQUEST_CODE, i, 0);
        alarmservice.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + 1000, pi); // wait 1 second
        //service.stopSelf();
        Log.v(TAG, "Killing ATS PID " + android.os.Process.myPid());
        android.os.Process.killProcess(android.os.Process.myPid());
    } // restartAtsProcess
    ...
    public void checkAdjustTime(long newTime_ms) {
        ...
        if ((c_new.get(Calendar.YEAR) >= 2015) &&
            ...
            service.addEvent(EventType.EVENT_TYPE_CHANGE_SYSTEMTIME);
        }
